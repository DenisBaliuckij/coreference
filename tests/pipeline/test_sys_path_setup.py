...
def test_default_path_points_at_sibling_dags_dir():
    ...
    # Implementation uses parents[2] from pipeline/sys_path_setup.py (1 level deep)
    # Test is at tests/pipeline/test_sys_path_setup.py (2 levels deep)
    # So test uses parents[3] to refer to the same directory
    assert result == Path(__file__).resolve().parents[3] / "text-corpuses-processing" / "dags"
...
def test_idempotent_does_not_duplicate_sys_path_entry(monkeypatch):
    monkeypatch.setattr(sys, "path", [p for p in sys.path if "text-corpuses-processing" not in p])
